...
export default class App extends React.Component {
  ...
  _slide = (authState=null) => {
    const animation = authState != null ? { 0: { transform: [{translateY: -100}] }, 1: { transform: [{translateY: 100}] } } : { 0: { opacity: 0 }, 1: { opacity: 1 } };
    this.view.animate(animation).then((this.setState({signUp: authState})));
  }
  ...
  render() {
    ...
    return (
      <Authentication>
        ...
        <TopView>
          <Title>boop</Title>
          <Tagline>Ask your friends to play games now. Or later.</Tagline>
        </TopView>
        ...
        <ButtonChoice>
          ...
          <AuthButton Primary large rounded right onPress={() => this._slide(true)}>
            <Text>Sign Up</Text>
          </AuthButton>
        </ButtonChoice>
          <View style={{flex: 1}} ref={this.handleViewRef}>
          <ModalContainer>
          <MenuContainer>
            <AuthInputs
